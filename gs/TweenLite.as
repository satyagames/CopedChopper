...
class gs.TweenLite {
	...
	var tweens:Array; //Contains parsed data for each property that's being tweened (each has to have a target, property, start, and change).
	...
	public static function executeAll():Void {
		...
		if (_listening) {
			...
			var tw:Object, twp:String, p:String;
			for (p in a) {
				for (twp in a[p]) {
					tw = a[p][twp];
					if (tw.active) {
	...
	public static function killGarbage():Void {
		if (_listening) {
			var a:Object = _all;
			var tw:Object, p:String, twp:String;
			var tg_cnt:Number = 0;
			var tw_cnt:Number = 0;
			for (p in a) {
				tw_cnt = 0;
